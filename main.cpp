...
sf::Sprite NewSprite(const std::string& piece_name)
{
	sf::Sprite piece;
...
sf::RectangleShape DrawSelectedSquare(sf::Event event)
{
...
sf::Sprite MovePiece(const std::string& selected_piece, sf::Event event)
{
...
bool IsTheSameRect(const sf::RectangleShape& rect_1, const sf::RectangleShape& rect_2)
{
	return (rect_1.getPosition().x / 75 == rect_2.getPosition().x / 75
			&& rect_1.getPosition().y / 75 == rect_2.getPosition().y / 75);
}
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			// When the mouse button is released, updates the new piece position
			...
			if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
			{
				...
				selected_square = DrawSelectedSquare(event);
			}
